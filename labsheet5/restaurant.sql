...
SELECT COMPANYNAME FROM VENDORS WHERE (referredby IN 
(SELECT vendorid FROM VENDORS WHERE companyname='vEGGIES_R_US') AND 
...
SELECT STOREID,SUM(PRICE) FROM ORDERS WHERE
menuitemid IN
(SELECT ITEMID FROM madewith WHERE ingredientid IN(
SELECT ingredientid FROM ingredients WHERE vendorid IN(
SELECT VENDORID FROM VENDORS WHERE companyname='vEGGIES_R_US')))
...
SELECT ingredientid FROM ingredients WHERE vendorid NOT IN(
SELECT VENDORID FROM VENDORS WHERE companyname='vEGGIES_R_US')
...
(SELECT I.unitprice FROM ingredients I JOIN MADEWITH M ON M.ingredientid=I.ingredientid 
JOIN items IT ON M.itemid=IT.itemid WHERE IT.name LIKE '%SALAD')
